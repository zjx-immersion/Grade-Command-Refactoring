...
class System {
    ...
    parseCommand(input) {
        ...
        if (input === INPUT_STUDENT) {
            result = {msg: ADD_STUDENT_MSG, status: ADD_STUDENT};
        } else if (input === INPUT_STUDENT_NUMBER) {
            result = {msg: QUERY_SCORE_MSG, status: QUERY_SCORE}
        } else if (input === CLOSE) {
            result = {msg: GOODBYE, status: TERMINATE}
            this.readline.close();
        } else {
            result = {msg: COMMAND_MSG, status: COMMAND};
        }
        return result;
    }
    ...
    parseInput(input) {
        ...
        if (this.consoleState === COMMAND) {
            let router = this.parseCommand(input)
            this.consoleState = router.status
            printResult = router.msg;
        } else if (this.consoleState === ADD_STUDENT) {
            let router = this.addStudent(input)
            this.consoleState = router.status;
            printResult = router.msg
        } else if (this.consoleState === QUERY_SCORE) {
            this.consoleState = COMMAND;
            printResult = this.queryScores(input) + "\r\n" + COMMAND_MSG;
        }
        return printResult
    }
