...
class System {
  ...
  updateClasses(student) {
    let exitedClazz = this.classes.find(clazz => clazz.classNumber === student.classNum);
    if (exitedClazz) {
      exitedClazz.addStudent(student).updateScores();
    } else {
      let clazz = new Class(student.classNum);
      clazz.addStudent(student).updateScores();
