...
class System {
  ...
  getClassesInfo(stuNumbers) {
    ...
    for (let stuNumber of stuNumbers) {
      for (let clazzInSystem of this.classes) {
        ...
        if (student) {
          let addedClazz = resultClasses.find(clazz => clazz.classNumber === student.classNumber);
          if (addedClazz) {
            addedClazz.addStudent(student);
          } else {
            const queryClass = this.createQueryClass(student, clazzInSystem);
            resultClasses.push(queryClass);
          }
        }
      }
    }
  ...
  createQueryClass(student, exitClazz) {
    let clazz = new Class(student.classNumber);
